...
(function () {
    ...
    /* Portal accent: early paint lives in default.hbs; re-run on theme toggle. */
    function stylePortalAccent() {
        if (typeof window.__gcPaintPortalAccent === 'function') {
            window.__gcPaintPortalAccent();
            return;
        }
        /* Fallback if inline script missing */
        var accents = window.__gcPortalAccents || {};
        var mode = currentTheme();
        var color = accents[mode] || getComputedStyle(root).getPropertyValue('--gc-accent').trim();
        if (!color) return;
        ...
        for (var i = 0; i < frames.length; i++) {
            try {
                ...
                if (btn) btn.style.setProperty('background', color, 'important');
                frames[i].classList.add('gc-portal-ready');
            } catch (e) { /* ignore */ }
        }
    }
